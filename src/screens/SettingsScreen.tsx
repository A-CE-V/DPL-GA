...
// FIX: no code changes below — this file was audited end-to-end and every
// setting already works correctly (verified: both toggles persist to
// localStorage AND are actually read by HomeScreen's auto-update/analytics
// logic; delete and clear-cache both call real, working backend functions).
// The reported "I don't know if they work" was a symptom of the scroll bug
// hiding half this screen, combined with low-contrast text making state
// changes hard to actually see — both fixed below without touching any
// working logic.
...
function ToggleRow({
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 16, opacity: disabled ? 0.4 : 1 }}>
      <div style={{ flex: 1 }}>
        <p style={{ fontFamily: "'DM Mono',monospace", fontSize: "var(--text-base)", color: "var(--text-primary)" }}>{label}</p>
        <p style={{ fontFamily: "'DM Mono',monospace", fontSize: "var(--text-xs)", color: "var(--text-muted)", marginTop: 3, lineHeight: 1.55 }}>{desc}</p>
      </div>
      <button
        type="button"
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "var(--text-base)", color: "var(--text-primary)", fontWeight: 600 }}>
          v{version.tag}
        </span>
        {info ? (
          <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "var(--text-xs)", color: "var(--text-muted)", marginLeft: 10 }}>
...
export function SettingsScreen({ config, versions, onBack, fromCache = false }: Props) {
  ...
  return (
    // ═══════════════════════════════════════════════════════════════════════
    // FIX — the scroll bug.
    // global.css sets overflow:hidden on html/body/#root. The old version
    // of this file was a single minHeight:100vh div with no scrollable
    // wrapper — content taller than the window had nowhere to scroll to.
    // This now mirrors the pattern HomeScreen already used correctly: a
    // fixed-height flex column (header) + a flex:1 scrollable body using
    ...
    <div style={{
      ...
      <div style={{
        ...
        padding: `calc(var(--safe-margin) + 6px) calc(var(--safe-margin) + 10px) 16px`,
        flexShrink: 0,
      }}>
        <button
          onClick={onBack}
        ...
        <div>
          <p style={{ fontFamily: "'Syne',sans-serif", fontSize: "var(--text-lg)", fontWeight: 800, color: "var(--text-primary)" }}>
            Settings
          </p>
        ...
        {savedPrefs && (
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 5, color: accent, fontSize: "var(--text-xs)" }}>
            <Check size={13} /> Saved
          </div>
        )}
      </div>
      ...
      {/* Body — scrollable */}
      <div className="dl-scroll-region" style={{ flex: 1, padding: `0 calc(var(--safe-margin) + 10px) calc(var(--safe-margin) + 20px)` }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 560 }}>
          ...
          {fromCache && (
            <div style={{
              ...
              <div>
                <p style={{ fontSize: "var(--text-sm)", color: "#eab308", fontWeight: 600 }}>Offline Mode</p>
                <p style={{ fontSize: "var(--text-xs)", color: "rgba(234,179,8,0.75)", marginTop: 2, lineHeight: 1.55 }}>
          ...
          <Section title="Installed Versions" Icon={HardDrive} accent={accent}>
            {localVersions.length === 0 ? (
              <p style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)" }}>No versions downloaded.</p>
            ) : (
          ...
          <Section title="Offline Cache" Icon={Clock} accent={accent}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
              <div>
                {cacheInfo ? (
                  ...
                  <p style={{ fontSize: "var(--text-base)", color: "var(--text-muted)" }}>No cache stored.</p>
                )}
              </div>
              {cacheInfo && (
                <button
                  ...
                  style={{
                    ...
                    color: "#f87171", fontFamily: "'DM Mono',monospace", fontSize: "var(--text-xs)", cursor: "pointer",
                  }}
                >
            ...
            <p style={{ fontSize: "var(--text-xs)", color: "var(--text-muted)", marginTop: 8, lineHeight: 1.55 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {[
                ...
                <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                  ...
                  <span style={{ fontSize: "var(--text-xs)", color: "var(--text-secondary)" }}>{value}</span>
                </div>
              ))}
            </div>
          </Section>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
        ...
        <p style={{ fontFamily: "'Syne',sans-serif", fontSize: "var(--text-base)", fontWeight: 700, color: "var(--text-primary)" }}>
          {title}
        </p>
      </div>
